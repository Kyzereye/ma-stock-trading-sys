...
from utils.performance_analyzer import get_top_performers_from_db, get_analysis_stats
import logging
...
def get_top_performers():
    ...
    try:
        logger.info("Getting top performers from pre-computed results")
        data = request.get_json()
        
        # Get parameters (for future filtering by analysis parameters)
        initial_capital = float(data.get('initial_capital', 100000))
        ...
        top_performers = get_top_performers_from_db(
            limit=10,
            analysis_params=analysis_params
        )
        
        if not top_performers:
            # If no pre-computed results, return empty with suggestion
            return jsonify({
                'success': True,
                'top_performers': [],
                'total_analyzed': 0,
        ...
        logger.info(f"Retrieved {len(top_performers)} top performers from database")
...
@ema_bp.route('/analysis-stats', methods=['GET'])
